...
INSERT INTO pedidos VALUES
 ...
 -- Descripción de tablas para crear la vista correctamente
...
select
	...
	-- Consulta de id_producto. nombre, categoria, precio de la tabla productos
	pr.producto_id,
    pr.nombre,
    pr.categoria,
    pr.precio,
    -- Consulta que cuenta todos los pedidos realizados que tengan clientes distintos
    count(distinct pe.cliente_id) as total_clientes_distintos
from productos pr
-- interseccion entre las tablas productos y pedidos los cuales tengan la misma id
inner join pedidos pe on pr.producto_id = pe.producto_id
-- los pedidos deben ser tener el estado 'entregado'
where pe.estado = 'entregado'
group by pr.producto_id, pr.nombre, pr.categoria, pr.precio
-- el total de clientes distintos que compraron un producto debe ser mayor a 1. 
having count(distinct pe.cliente_id) > 1;

-- consulta que muestra todas las columnas de la vista unicamente para los productos en la categoria 'perifericos'
