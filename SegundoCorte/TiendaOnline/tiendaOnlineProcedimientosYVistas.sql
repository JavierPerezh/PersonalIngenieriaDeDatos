...
-- Vista para consultar pedidos con detalles de cliente y producto
...
-- Vista para consultar pedidos cancelados y su impacto en stock
CREATE OR REPLACE VIEW vw_pedidos_cancelados AS
SELECT 
    p.id_pedido,
    p.fecha_pedido,
    p.estado,
    ...
    pr.id_producto,
    pr.nombre AS nombre_producto,
    dp.cantidad AS cantidad_devuelta,
    dp.precio_unit,
    pr.stock AS stock_actual,
    (pr.stock - dp.cantidad) AS stock_antes_cancelacion
FROM pedidos p
INNER JOIN clientes c ON p.id_cliente = c.id_cliente
INNER JOIN detalle_pedido dp ON p.id_pedido = dp.id_pedido
INNER JOIN productos pr ON dp.id_producto = pr.id_producto
WHERE p.estado = 'cancelado'
ORDER BY p.fecha_pedido DESC, p.id_pedido;

SELECT * FROM vw_pedidos_cancelados;
